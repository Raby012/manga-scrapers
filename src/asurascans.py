...
class Asurascans:
    ...
    def info(self, id: str):
        try:
            ...
            content["type"] = soup.select_one(
                "tr:nth-child(2) > td:nth-child(2)"
            ).get_text()
            content["year"] = soup.select_one(
                "tr:nth-child(3) > td:nth-child(2)"
            ).get_text()
            content["author"] = (
                soup.select_one("tr:nth-child(4) > td:nth-child(2)")
                .get_text()
                .split(",")
            )
            content["artists"] = (
                soup.select_one("tr:nth-child(5) > td:nth-child(2)")
                .get_text()
                .split(",")
            )
            content["serialization"] = (
                soup.select_one("tr:nth-child(6) > td:nth-child(2)")
                .get_text()
                .split(",")
            )
            ...
            for items in chapterSelector:
                tempChapter = {}
                tempChapter["title"] = items.find(
                    "span", class_="chapternum"
                ).get_text()
                tempChapter["date"] = items.find(
                    "span", class_="chapterdate"
                ).get_text()
