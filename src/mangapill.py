...
class Mangapill:
    ...
    def search(self, query: str):
        try:
            ...
            for items in cards:
                tempContent = {}
                tempContent["id"] = (
                    items.find("a", class_="relative block")
                    .get("href")
                    .split("/", 1)[1]
                )
                tempContent["title"] = items.find(
                    "div", class_="mt-3 font-black leading-tight line-clamp-2"
                ).get_text()
                try:
                    tempContent["subheading"] = items.find(
                        "div", class_="line-clamp-2 text-xs text-secondary mt-1"
                    ).get_text()
                except:
                ...
                tempContent["image"] = (
                    ...
                    .find("img")
                    .get("data-src")
                )  # MARK: Referer is required
                genresSelector = items.find(
                    "div", class_="flex flex-wrap gap-1 mt-1"
                ).find_all("div")
    ...
    def recent(self):  # Same as search
        try:
            ...
            cards = soup.select(
                "body > div.container.py-3 > div.grid.grid-cols-2 > div"
            )

            for items in cards:
                tempContent = {}
                tempContent["id"] = (
                    items.find("div", class_="px-1")
                    .find("a", class_="mt-1.5 leading-tight text-secondary")
                    .get("href")
                    .split("/", 1)[1]
                )
                tempContent["image"] = (
                    items.find("a").find("figure").find("img").get("data-src")
                )
                tempContent["title"] = (
                    items.find("div", class_="px-1")
                    .find("a", class_="mt-1.5 leading-tight text-secondary")
                    .find("div", class_="line-clamp-2 text-sm font-bold")
                    .get_text()
                )
                self.results["results"].append(tempContent)
